/*=======================================================================
|   Source code:  FastFibSequence.java
|
|         Class:  FastFibSequence 
|
|        Author:  Richard Molina
|    Student ID:  6140544
|    Assignment:  Program #6 - Recursive Fibonacci
|  
|            Course:  COP 3337 (Intermediate Programming)
|           Section:  U08
|        Instructor:  William Feild  
|          Due Date:  29 November 2018, at the beginning of class 
|
|	I hereby certify that this collective work is my own
|	and none of it is the work of any other person or entity.
|	______________________________________ [Signature]
|  
|     Language:  Java 
|  Compile/Run:   
| 		javac FastFibSequence.java Sequence.java
| 
|        Purpose:  This class serves to implement the Sequence interface to
|				   allow for the construction of FastFibSequence objects. The class
|				   provides methods to construct a new FastFibSequence, call the next
|				   value in the sequence, and calculate the next Fibonacci number
|				   based on the previous/starting base.
|
|  Inherits From:  N/A
|
|     Interfaces:  Sequence.java
|
|  +-----------------------------------------------------------------------
|
|      Constants:  N/A
|
| +-----------------------------------------------------------------------
|
|     Constructors:  FastFibSequence
|                     - Constructs a FastFibSequence object
|					  - Takes input for the starting number of the sequence,
|						storing it as one less than the input value, because
|						the next() method begins by incrementing the current
|						base
|					  - Takes input for the length of the sequence, constructing 
|						an int[] so that there is an index for every Fibonacci 
|						base in the sequence.
|
|    Class Methods:  fibonacci() 
|					  - Calculates the Fibonacci number for an input base
|					  - Utilizes a fast recursive formula
|					  - Returns the calculated Fibonacci number
|
| Instance Methods:  next() 
|					  - Iterates the current base
|					  - Returns the next Fibonacci number in the sequence (based
|						on that new current base)
|
|  *===========================================================================*/

public class FastFibSequence implements Sequence
{
	private int currentBase;
	private int[] answers;
	
	/*---------------------------- FastFibSequence ----------------------------
    |  Constructor FastFibSequence (startingNumber, sequenceLength)
    |
    |  Purpose:  To construct a FastFibSequence object. Takes input for the 
    |			 starting number of the sequence, storing it as one less 
    |			 than the input value, because the next() method begins by 
    |			 incrementing the current base. Also takes input for the length 
    |			 of the sequence, constructing an int[] so that there is an 
    |			 index for every Fibonacci base in the sequence.
    |
    |  @param	 startingNumber the first base of the sequence
    |			 sequenceLength the length of the sequence
    *-------------------------------------------------------------------*/
	public FastFibSequence(int startingNumber, int sequenceLength)
	{
		currentBase = startingNumber - 1;
		answers = new int[sequenceLength + startingNumber + 1];
	}
	
	/*---------------------------- next ----------------------------
    |  Method next ()
    |
    |  Purpose:  To iterate the current base of the Fibonacci sequence.
	|
    |			 Implements the next() method from the Sequence interface.
    |
    |  @param	 
    |
    |  @return   the next Fibonacci number in the sequence (based on that 
    |			 new current base)
    *-------------------------------------------------------------------*/
	public int next() 
	{
		currentBase++;
		return fibonacci(currentBase);
	}
	
	/*---------------------------- fibonacci ----------------------------
    |  Method fibonacci (base)
    |
    |  Purpose:  To calculate the Fibonacci number for an input base.
    |			 A Fibonacci number is calculated by summing the previous 
    |			 two Fibonacci numbers in the sequence, where the first two
    |			 numbers in the sequence are 1 and 1. The base number describes 
    |			 the position of the desired Fibonacci number in the sequence.
    |			 Utilizes a fast recursive formula, storing calculated values
    |			 an int[], preventing repetitive calls by searching the array
    |			 at [base] before calculating a new value. 
	|
	|     		 Source:  javabrahman.com/gen-java-programs/
	|						   recursive-fibonacci-in-java-with-memoization/
    |
    |  @param	 base the input base number
    |
    |  @return   the calculated Fibonacci number
    *-------------------------------------------------------------------*/
	private int fibonacci(int base)
	{
		final int BASE_CASE = 1;
		final int FIRST_NTH_BASE = base - 1;
		final int SECOND_NTH_BASE = base - 2;
		final int EMPTY_ANSWER = 0;
		
		if (base <= BASE_CASE) 
		{
		  return base;
		}
		else if (answers[base] != EMPTY_ANSWER)
		{
			return answers[base];
		}
		else
		{
		    int fibNumber = fibonacci(FIRST_NTH_BASE) + fibonacci(SECOND_NTH_BASE);
		    answers[base] = fibNumber;
		    return fibNumber;
		}
	}
}